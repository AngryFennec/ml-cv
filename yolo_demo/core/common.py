...
def EP(
    input_data, input_channels, output_channels, name, stride=1, trainable=True
):
    with tf.variable_scope(name):
        x = input_data
        input_data = sepconv(
...
def FCA_A(input_data, channels, reduction_ratio):
    ...
    input_data = fcn_layer(
        input_data, channels, hidden_channels, activation=tf.nn.relu
    )
    input_data = fcn_layer(
        input_data, hidden_channels, channels, activation=tf.sigmoid
    )
...
def FCA(input_data, channels, reduction_ratio):
    x = input_data
    hidden_channels = channels // reduction_ratio
    input_data = tf.layers.dense(
        input_data, hidden_channels, activation=tf.nn.relu
    )
    input_data = tf.layers.dense(
        input_data, channels, activation=tf.nn.sigmoid
    )
    return x * input_data
